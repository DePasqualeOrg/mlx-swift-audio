...
class CosyHiFTGenerator: Module {
  ...
  func decode(x: MLXArray, s: MLXArray) -> MLXArray {
    // Debug helper
    func debugArray(_ name: String, _ arr: MLXArray) {
      arr.eval()
      let flat = arr.reshaped(-1)
      let minVal = MLX.min(flat).item(Float.self)
      let maxVal = MLX.max(flat).item(Float.self)
      print("[HiFiGAN DEBUG] \(name): shape=\(arr.shape), min=\(minVal), max=\(maxVal)")
    }

    debugArray("input mel (x)", x)
    debugArray("source signal (s)", s)
    ...
    debugArray("sStft after STFT", sStft)

    // Pre-convolution
    var h = x.swappedAxes(1, 2)
    h = convPre(h)
    h = h.swappedAxes(1, 2)
    debugArray("after convPre", h)
    ...
    for i in 0 ..< numUpsamples {
      ...
      debugArray("after sourceDowns[\(i)]", si)
      si = sourceResblocks[i](si)
      debugArray("after sourceResblocks[\(i)]", si)
      h = h + si
      debugArray("after source fusion[\(i)]", h)
      ...
      debugArray("after resblocks[\(i)]", h)
    }
