...
actor CosyVoice2TTS {
  ...
  /// Load CosyVoice2TTS from model path
  /// - Parameters:
  ...
  static func load(
    modelPath: String,
    progressHandler _: @escaping @Sendable (Progress) -> Void = { _ in }
  ) async throws -> CosyVoice2TTS {
    // Load configuration
    let config = try CosyVoice2Config.fromPretrained(modelPath: modelPath)

    // Load model weights
    let modelURL = URL(fileURLWithPath: modelPath).appendingPathComponent("model.safetensors")
    ...
    let textTokenizer = try loadTokenizer(modelPath: modelPath)

    return CosyVoice2TTS(model: model, speakerEncoder: speakerEncoder, textTokenizer: textTokenizer)
  }

  /// Load Qwen2 tokenizer from model path
  /// - Parameter modelPath: Path to the model directory containing tokenizer.json and tokenizer_config.json
  /// - Returns: PreTrainedTokenizer configured for CosyVoice2
  private static func loadTokenizer(modelPath: String) throws -> PreTrainedTokenizer {
    ...
    guard FileManager.default.fileExists(atPath: tokenizerConfigURL.path) else {
      throw CosyVoice2Error.tokenizerNotFound("tokenizer_config.json not found at \(modelPath)")
    }
    guard FileManager.default.fileExists(atPath: tokenizerDataURL.path) else {
      throw CosyVoice2Error.tokenizerNotFound("tokenizer.json not found at \(modelPath)")
    }
  ...
  private static func createModel(
    ...
    let llmWeights = weights.filter { $0.key.hasPrefix("llm.") }
      .reduce(into: [:]) { result, pair in
    ...
    let qwen2Weights = weights.filter { $0.key.hasPrefix("qwen2.") && $0.key != "qwen2.lm_head.weight" }
      .reduce(into: [:]) { result, pair in
    ...
    // Load flow weights
    let flowWeights = weights.filter { $0.key.hasPrefix("flow.") }
      .reduce(into: [:]) { result, pair in
        result[String(pair.key.dropFirst("flow.".count))] = pair.value
      }
  ...
  func prepareConditionals(
    ...
    mel80.eval()
    let mel80Min = MLX.min(mel80).item(Float.self)
    let mel80Max = MLX.max(mel80).item(Float.self)
    let mel80Mean = MLX.mean(mel80).item(Float.self)
    print("[prepareConditionals] mel80 shape: \(mel80.shape), min: \(mel80Min), max: \(mel80Max), mean: \(mel80Mean)")
