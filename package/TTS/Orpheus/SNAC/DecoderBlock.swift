...
struct DecoderBlock {
  ...
  init(inputDim _: Int, outputDim: Int, stride: Int, noise: Bool, groups: Int, weights: [String: MLXArray], basePath: String) {
    ...
    if noise {
      noiseBlock = NoiseBlock(dim: outputDim, weights: weights, basePath: "\(basePath).2.linear")
    ...
    residualUnit1 = ResidualUnit(dim: outputDim, dilation: 1, kernelSize: 7, groups: groups, weights: weights, basePath: resBasePath1)

    let resBasePath2 = "\(basePath).\(residualUnitBasePathStartIndex + 1).block.layers"
    residualUnit2 = ResidualUnit(dim: outputDim, dilation: 3, kernelSize: 7, groups: groups, weights: weights, basePath: resBasePath2)

    let resBasePath3 = "\(basePath).\(residualUnitBasePathStartIndex + 2).block.layers"
    residualUnit3 = ResidualUnit(dim: outputDim, dilation: 9, kernelSize: 7, groups: groups, weights: weights, basePath: resBasePath3)
  }
