...
class Dequantizer {
  static func dequantize(_ w_q: MLXArray, scales: MLXArray, biases: MLXArray, groupSize: Int, bits: Int) -> MLXArray {
    guard bits == 4 else {
    ...
    if w_q.dtype == .uint32 {
      ...
      Log.model.warning("Converting quantized weights from \(String(describing: w_q.dtype)) to uint32")
